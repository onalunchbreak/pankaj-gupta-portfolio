...
export default function Hero() {
  ...
  // Render a single tag/annotation canvas tile
  const renderTagsTile = (keySuffix: string) => (
    <div key={keySuffix} className="absolute inset-0 w-full h-full pointer-events-none">
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 my-auto flex w-full flex-1 items-center justify-center py-1">
        ...
        {/* ---- INFINITE REPEATING DESKTOP ANNOTATIONS & OUTLINED TAGS LAYER ---- */}
        <div className="pointer-events-none absolute inset-0 z-20 hidden lg:block overflow-visible">
          {/* Tile -1 (Left Tile) */}
          <motion.div
            className="absolute inset-0 w-full h-full"
            animate={{ x: `calc(${xOffset}px - 100%)` }}
            transition={{ duration: 0.15, ease: "easeInOut" }}
          >
            {renderTagsTile("tile-left")}
          </motion.div>
      ...
      <div className="fixed bottom-4 right-16 z-[80] pointer-events-auto">
        {showControls ? (
          ...
          >
            ...
            {/* X-Offset Control (Infinite Repeating Loop) */}
            <div className="flex flex-col gap-1 mt-1">
              <div className="flex justify-between text-[10px] text-white/80">
